...
#else
			out_socket_(in_socket.get_executor()/*.context()*/),
			resolver(in_socket.get_executor()/*.context()*/),
#endif
			in_buf_(buffer_size), 
...
{
public:
	Server(boost::asio::io_service& io_service, short port, unsigned buffer_size, short verbose_level)
		: acceptor_(io_service, tcp::endpoint(tcp::v4(), port)), 
		in_socket_(io_service), buffer_size_(buffer_size), verbose_level_(verbose_level), session_id_(0)
	{
		do_accept();
	}
	...
	void do_accept()
	{
		acceptor_.async_accept(in_socket_,
			[this](boost::system::error_code ec)
			{
				if (!ec)
				{
					std::make_shared<Session>(std::move(in_socket_), session_id_++, buffer_size_, verbose_level_)->start();
				}
				else
					write_log(1, 0, verbose_level_, session_id_, "socket accept error", ec.message());
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		unsigned int/*size_t*/ buffer_size = conf.check_key("buffer_size") ? std::atoi(conf.get_key_value("buffer_size")) : 8192; // Default buffer_size
		verbose_level = conf.check_key("verbose_level") ? std::atoi(conf.get_key_value("verbose_level")) : 0; // Default verbose_
#else
		short port = 19002;
		...
		Server server(io_service, port, buffer_size, verbose_level);
		io_service.run();
	}
	catch (std::exception& e)
	{
		write_log(1, 0, verbose_level, -1, "", e.what());
	}
	catch (...)
	{
		write_log(1, 0, verbose_level, -1, "", "exception...");
	}
